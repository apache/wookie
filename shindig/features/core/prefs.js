...
/// WOOKIE ADDITION
/**
 * Loads the persisted preferences from Wookie
 */
function loadPrefs(){
    Widget.init();
	for (var x=0;x<Widget.preferences.length;x++){
		key = Widget.preferences.key(x);
		prefs[key] = Widget.preferences.getItem(key);
	}
};
...
gadgets.Prefs.setInternal_ = function(key, value) {
  if (typeof key === "string") {
    ...
    for (var k in key) if (key.hasOwnProperty(k)) {
      prefs[k] = key[k];
	  /// WOOKIE ADDITION
	  Widget.preferences.setItem(k,key[k]);
    }
  }
};
