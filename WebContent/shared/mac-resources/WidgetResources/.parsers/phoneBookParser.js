...
{   
	...
	else if( requestKind=="locationValidation" )
	{
		...
		alert("Phone Book Widget: unknown query task: " + searchData.queryTask );
	
	xml_request.onerror = function() { alert("Phone Book Widget: xml request failed."); }
		...
		else
		{
			...
			else
			{
				try
				{
					...
					{
						alert("Phone Book: Unexpected results format returned for validation query.");
...
{	
	...
	if( topResults )
	{
		...
		if( dataList )
		{
			...
			alert("Phone Book Widget: failed to extract data from XML response.  Can't get: " + itemListID + " from : " + topResultsID);
	}
...
{   
    ...
    var newBusiness = {name:name, address:address, city:city, state:state, zip:zip, phone:phone, distance:distance, mapLink:mapLink, encrypted_id:encrypted_id};
...
function buildSearchRequest( searchData, addressInfo )
{
    ...
    if( searchData.queryType=="search" && queryAddressZip.length <= 0 )
    {
    	queryAddressCity = ( addressInfo != null && (addressInfo[1].length > 0)) ? addressInfo[1] : "Cupertino";
	...
	var queryURL = "http://www.daplus.us/apple/GetResults.aspx?SearchType=Business";
...
{
	return "http://www.daplus.us/ViewBusinessProfile.aspx?recordid=" + encrypted_id + "&Partner=400119";
}
