...
function patchedSetupUpdateTrackFlight(animate)
{
	...
	var jsFunc = "performXMLRequest(trackingAirlineCode, null, defaultDepartCity, defaultArriveCity, true, trackingDataLoaded)";
	lastXMLRequest = eval(jsFunc);
	timerInterval = setInterval(jsFunc, 30*60000); // every 30 minutes
}

function performXMLRequest (airlineCode, flightNumber, departAirport, arriveAirport, detail, callback)
{
	if (firstRequest) {
		window.setupUpdateTrackFlight = patchedSetupUpdateTrackFlight;
		firstRequest = false;
	}
	var xml_request = new XMLHttpRequest();
	
	var postData = "Entity=Flight&ContractID=490&Enroute=N&FlightCategory=COM";
	//&DepartureDate=2005-02-26&DepartureDate=2005-02-26";
	
	if ( detail == true )
	    postData = postData + "&DataView=detail";
    else
        postData = postData + "&DataView=summary";
   	var departCode = (departAirport.code != undefined ? departAirport.code : departAirport);
   
    if (departAirport && departCode.length)
        postData = postData + "&DepartureAirportCode=" + departCode;
    
   	var arriveCode = (arriveAirport.code != undefined ? arriveAirport.code : arriveAirport);
   
    if (arriveAirport && arriveCode.length)
        postData = postData + "&ArrivalAirportCode=" + arriveCode;
	...
	xml_request.open("POST", "http://applecomputer:apc0606jb@flytesource.flytecomm.com/FlyteSourceGeneratorWebApp/Application");
...
function xml_loaded (event, request, callback)
{
	...
	if (request.responseXML)
	{
		...
		var data = {flights:new Array, airports:(airportList==null?null:new Array), error:0};
